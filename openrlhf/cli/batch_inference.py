...
from openrlhf.utils.device_utils import current_device, device_count
...
def batch_rm_inference(args):
    ...
    with torch.no_grad():
        for _, input_ids, attention_masks, info in pbar:
            input_ids = input_ids.squeeze(1).to(current_device())
            attention_masks = attention_masks.squeeze(1).to(current_device())
...
if __name__ == "__main__":
    ...
    # For vllm
    parser.add_argument("--tp_size", type=int, default=device_count())
